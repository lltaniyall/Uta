...
TOPのボタンを以下の3つに変更しました。

- LATEST CONTENTS ↓ → 最新のコンテンツへ
- 曲を探す ↓ → EXPLOREへ
- SONG DATABASE ↗ → 曲データベースへ
...
- アーカイブタイトルを小さめに調整
- SHAREボタンはサイト内URLではなく、実際のYouTubeアーカイブURLをコピー
- 再生中の場合は現在位置を `&t=秒数s` として付与
- 0秒の場合は通常のYouTube動画URLをコピー
- コピー完了/失敗メッセージを右下トーストに変更
- トーストはページ内UIなので、ブラウザのポップアップ許可は不要
